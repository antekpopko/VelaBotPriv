...
module.exports.config = { name: "flux", version: "2.0", hasPermssion: 0, credits: "Dipto (tłumaczenie i poprawki: January Sakiewka)", description: "Generator obrazów Flux", commandCategory: "🖼️ Generator Obrazów", usage: "{pn} [opis] --ratio 1024x1024\n{pn} [opis]", countDown: 15, };

module.exports.run = async ({ event, args, api }) => { const fluxAPI = "https://www.noobs-api.rf.gd/dipto";

try { const input = args.join(" "); const [promptText, ratio = "1:1"] = input.includes("--ratio") ? input.split("--ratio").map(s => s.trim()) : [input, "1:1"];
...
} catch (e) { console.error(e); return api.sendMessage("❌ Wystąpił błąd: " + e.message, event.threadID, event.messageID); } };
